...
async def generate_title(
    ...
    if not ENABLE_TITLE_GENERATION:
        return JSONResponse(
    ...
    log.debug(
        f"generating chat title using model {model_id} for user {user.id} "
    )
...
async def generate_chat_tags(
    ...
    model_id = form_data["model"]
    log.debug(
        f"generating chat tags using model {model_id} for user {user.id} "
    )
...
async def generate_autocompletion(
    ...
    if request.app.state.config.AUTOCOMPLETE_GENERATION_INPUT_MAX_LENGTH > 0:
        if len(prompt) > request.app.state.config.AUTOCOMPLETE_GENERATION_INPUT_MAX_LENGTH:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Input prompt exceeds maximum length of "
                       f"{request.app.state.config.AUTOCOMPLETE_GENERATION_INPUT_MAX_LENGTH}",
            )

    # Build the models with the corresponding name
    model_id = form_data["model"]
