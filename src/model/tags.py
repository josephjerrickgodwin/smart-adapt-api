...
from src.model.db import Base, get_db
...
class TagTable:
    @staticmethod
    def insert_new_tag(name: str, user_id: str) -> Optional[TagModel]:
        with get_db() as db:
    ...
    @staticmethod
    def get_tag_by_name_and_user_id(name: str, user_id: str) -> Optional[TagModel]:
        try:
            id = name.replace(" ", "_").lower()
            with get_db() as db:
    ...
    @staticmethod
    def get_tags_by_user_id(user_id: str) -> list[TagModel]:
        with get_db() as db:
            return [
    ...
    @staticmethod
    def get_tags_by_ids_and_user_id(ids: list[str], user_id: str) -> list[TagModel]:
        with get_db() as db:
            return [
                TagModel.model_validate(tag)
                for tag in (
    ...
    @staticmethod
    def delete_tag_by_name_and_user_id(name: str, user_id: str) -> bool:
        try:
            with get_db() as db:
                id = name.replace(" ", "_").lower()
